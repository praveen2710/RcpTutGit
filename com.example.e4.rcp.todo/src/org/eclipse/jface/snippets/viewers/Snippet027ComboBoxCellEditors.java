...
public class Snippet027ComboBoxCellEditors {
	private class MyCellModifier implements ICellModifier {
		...
		public Object getValue(Object element, String property) {
			...
			System.out.println(((ContactDetails)element).getContactNumber());
	...
	private List<ContactDetails> createModel() {
		List<ContactDetails> elements = new ArrayList<ContactDetails>();

		for (int i = 0; i < 10; i++) {
			ContactDetails cd = new ContactDetails();
			cd.setContactNumber(i);
			cd.setPersonName(i+"name");
			elements.add(cd);
		}
		return elements;
	}
